...
export function renderAssignmentForm(ctx:Context){
  ...
  const addResource=(name="",url="")=>{const row=document.createElement("div");row.className="attachment-row";row.innerHTML='<label>ชื่อเอกสาร<input required maxlength="100" data-resource-name placeholder="เช่น โจทย์ Sprint Review" value="'+e(name)+'"></label><label>URL<input required type="url" data-resource-url placeholder="https://" value="'+e(url)+'"></label><button type="button" class="icon-button" aria-label="ลบเอกสาร">'+icon("close")+'</button>';row.querySelector("button")!.onclick=()=>{row.remove();form.dispatchEvent(new Event("input"));};resourceRows.append(row);};
  task?.resources.forEach(resource=>addResource(resource.name,resource.url));
  ...
  form.onsubmit=async event=>{
    ...
    try{
      ...
      const subject=catalog.subjects.find(row=>row.id===String(fd.get("subject_id")));
      const mode:AssignmentInput["schedule_mode"]=subject?.sectionCount===1?"UNIFIED":fd.get("mode") as AssignmentInput["schedule_mode"];
      const subjectName=subject?.name??(fd.get("subject_id")==="legacy-subject"?task?.subject_name:""),channelName=String(fd.get("submission_channel")??"").trim();
      if(!subjectName||!channelName)throw new Error("กรุณาเลือกรายวิชาและระบุช่องทางส่งงาน");
