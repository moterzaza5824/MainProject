...
const normalizedTaskStatus=(status?:string):TaskStatus=>status==="DONE"?"DONE":"TODO";
const taskStatusLabel=(status?:string)=>normalizedTaskStatus(status)==="DONE"?"เสร็จแล้ว":"ยังไม่เสร็จ";
...
export function taskCard(task: AssignmentRow, ctx: Context, section: Section = "ALL") {
  const progress = progressFor(ctx.data.progress, ctx.user.uid, task.assignment_id), status = normalizedTaskStatus(progress?.status);
...
export function calendarMarkup(tasks: AssignmentRow[], ctx: Context, month: Date, section: Section) {
  ...
  for(let i=0;i<42;i++){
    ...
    cells += `<div class="day ${date.getMonth()!==m?"outside":""} ${key(date)===todayKey?"today":""}"><span class="day-number">${date.getDate()}</span>${events.map(event=>{
      const progress=progressFor(ctx.data.progress,ctx.user.uid,event.task.assignment_id),status=normalizedTaskStatus(progress?.status);
      const eventSection=event.label.startsWith("Sec ")?event.label.slice(4) as Section:"ALL";
      return `<a class="calendar-event ${status.toLowerCase()} ${taskUrgency(event.task,progress,eventSection)}" href="${href("assignmentDetail",event.task.assignment_id)}">${e(event.task.title)}<br>${e(event.label)} · ${taskStatusLabel(status)}</a>`;
    }).join("")}</div>`;
  }
  ...
  <div class="legend">${statusBadge("TODO")}${statusBadge("DONE")}${badge("ขอบแดง: งานด่วน / เลยกำหนด","urgent")}</div></div>`;
}
