...
export function postCard(post: PostRow) {
  return `<article class="post-card ${post.is_pinned?"pinned":""}"><div class="post-card-head"><span class="author"><span class="avatar">${e(post.author_name.slice(0,1))}</span><span><strong>${e(post.author_name)}</strong><small class="post-date">${e(formatDate(post.created_at,false))}</small><small class="post-time">เวลา ${e(formatTime(post.created_at))} น.</small></span></span><div class="post-card-flags" aria-label="รายวิชา กลุ่มผู้รับ และสถานะ">${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div></div><div class="post-card-kind">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${post.status!=="published"?badge(postStatusLabel(post),post.status):""}</div><h2><a href="${href("postDetail",post.post_id)}">${e(post.title)}</a></h2><p>${e(post.content.replace(/[#*]/g,"").slice(0,150))}${post.content.length>150?"…":""}</p>${postImage(post,"post-card-image")}<div class="post-card-footer"><a class="post-read-more" href="${href("postDetail",post.post_id)}" aria-label="อ่าน ${e(post.title)}">ดูรายละเอียด ${icon("arrow")}</a></div></article>`;
}
