...
import { formatDate, formatTime } from "../utils/tasks";
...
const plainPostContent=(content:string)=>content.replace(/[#*`]/g,"").trim();
const postCopy=(post:PostRow)=>{const content=plainPostContent(post.content),lines=content.split(/\r?\n/).length;return `<div class="post-card-copy"><p data-post-summary>${e(content)}</p><p data-post-full hidden>${e(content)}</p><button class="post-copy-toggle" type="button" data-expand-post="${lines>10?"detail":"inline"}" data-post-id="${e(post.post_id)}" aria-expanded="false">ดูเพิ่มเติม</button></div>`;};
const syncPostCopyControls=(root:HTMLElement)=>root.querySelectorAll<HTMLElement>(".post-card-copy").forEach(copy=>{const summary=copy.querySelector<HTMLElement>("[data-post-summary]"),toggle=copy.querySelector<HTMLButtonElement>("[data-expand-post]");if(summary&&toggle&&summary.clientHeight>0)toggle.hidden=summary.scrollHeight<=summary.clientHeight+1;});
const exceedsTenRenderedLines=(copy:HTMLElement,full:HTMLElement)=>{const width=copy.clientWidth;if(width)full.style.width=`${width}px`;full.hidden=false;full.classList.add("post-copy-measure");const style=window.getComputedStyle(full),lineHeight=Number.parseFloat(style.lineHeight),tooLong=lineHeight>0&&full.scrollHeight/lineHeight>10;full.classList.remove("post-copy-measure");full.style.removeProperty("width");full.hidden=true;return tooLong;};
const postImage=(post:PostRow,className:string)=>{const url=post.image_url?safeUrl(post.image_url):null;return url?`<button class="post-image-button" type="button" data-image-post="${e(post.post_id)}" aria-label="เปิดดูภาพเต็มของ ${e(post.title)}"><img class="${className}" src="${e(url)}" alt="ภาพประกอบ ${e(post.title)}" loading="lazy" decoding="async"><span class="image-open-hint">กดเพื่อดูภาพเต็ม</span></button>`:"";};
const postAttachments=(post:PostRow)=>{const links=post.attachments.map(attachment=>externalLink(attachment.url,attachment.name)).filter(Boolean);return links.length?`<section class="post-card-attachments" aria-label="ลิงก์เอกสารแนบ"><h3>${icon("link")} ลิงก์เอกสารแนบ</h3>${links.join("")}</section>`:"";};
const openPostImage=(post:PostRow)=>{const safe=post.image_url?safeUrl(post.image_url):null;if(!safe)return;const attachments=post.attachments.length?`<section class="image-viewer-attachments"><h4>ลิงก์เอกสารแนบ</h4>${post.attachments.map(a=>externalLink(a.url,a.name)).join("")}</section>`:"";const viewer=dialog(`ดูรูปภาพจาก ${post.title}`,`<div class="image-viewer-layout"><div class="image-viewer-stage"><img class="image-viewer-image" src="${e(safe)}" alt="ภาพประกอบ ${e(post.title)}"></div><aside class="image-viewer-info"><span class="author"><span class="avatar">${e(post.author_name.slice(0,1))}</span><span><strong>${e(post.author_name)}</strong><small class="post-date">${e(formatDate(post.created_at,false))}</small><small class="post-time">เวลา ${e(formatTime(post.created_at))} น.</small></span></span><div class="image-viewer-badges">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div><h3>${e(post.title)}</h3><div class="prose">${markdown(post.content)}</div>${attachments}</aside></div>`,true);viewer.classList.add("image-viewer");};
export function postCard(post: PostRow) {
  return `<article class="post-card ${post.is_pinned?"pinned":""}"><div class="post-card-head"><span class="author"><span class="avatar">${e(post.author_name.slice(0,1))}</span><span><strong>${e(post.author_name)}</strong><small class="post-date">${e(formatDate(post.created_at,false))}</small><small class="post-time">เวลา ${e(formatTime(post.created_at))} น.</small></span></span><div class="post-card-flags" aria-label="รายวิชา กลุ่มผู้รับ และสถานะ">${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div></div><div class="post-card-kind">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${post.status!=="published"?badge(postStatusLabel(post),post.status):""}</div><h2><a href="${href("postDetail",post.post_id)}">${e(post.title)}</a></h2>${postCopy(post)}${postImage(post,"post-card-image")}${postAttachments(post)}<div class="post-card-footer"><a class="post-read-more" href="${href("postDetail",post.post_id)}" aria-label="อ่าน ${e(post.title)}">ดูรายละเอียด ${icon("arrow")}</a></div></article>`;
}
...
export function renderPostDetail(ctx:Context){
  ...
  `<div class="detail-grid"><article class="panel"><div class="post-detail-head"><span class="author"><span class="avatar">${e(post.author_name.slice(0,1))}</span><span><strong>${e(post.author_name)}</strong><small class="post-date">${e(formatDate(post.created_at,false))}</small><small class="post-time">เวลา ${e(formatTime(post.created_at))} น.</small></span></span><div class="task-top">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${badge(postStatusLabel(post),post.status)}${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div></div><h2 class="detail-title">${e(post.title)}</h2><div class="prose">${markdown(post.content)}</div>${postImage(post,"post-detail-image")}<section class="detail-section"><h3>ลิงก์เอกสารแนบ</h3>${post.attachments.length?post.attachments.map(a=>externalLink(a.url,a.name)).join(""):'<p class="note-hint">ไม่มีเอกสารแนบ</p>'}</section></article>
  ...
  ctx.root.querySelector<HTMLButtonElement>("[data-image-post]")?.addEventListener("click",()=>openPostImage(post));
