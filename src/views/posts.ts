...
export function renderPostDetail(ctx:Context){
  ...
  `<div class="detail-grid"><article class="panel"><div class="task-top">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${badge(postStatusLabel(post),post.status)}${post.is_pinned?badge("ปักหมุด","done"):""}</div><h2 class="detail-title">${e(post.title)}</h2><div class="detail-meta"><span>โดย ${e(post.author_name)}</span><span>${e(formatDate(post.created_at))}</span></div><div class="prose">${markdown(post.content)}</div><section class="detail-section"><h3>ลิงก์เอกสารแนบ</h3>${post.attachments.length?post.attachments.map(a=>externalLink(a.url,a.name)).join(""):'<p class="note-hint">ไม่มีเอกสารแนบ</p>'}</section></article>
  <aside class="stack"><div class="panel"><h2>ข้อมูลประกาศ</h2><dl class="profile-fields" style="display:block"><dt>อัปเดตล่าสุด</dt><dd>${e(formatDate(post.updated_at))}</dd>${post.approved_by?'<dt style="margin-top:16px">ผู้ดำเนินการ</dt><dd>'+e(reviewer?.full_name??"ผู้ดูแลระบบ")+"</dd>":""}</dl>${editable?'<div class="stack"><a class="button" href="'+href(ctx.user.role==="admin"?"adminPostForm":"postForm",post.post_id)+'">'+icon("edit")+' แก้ไขประกาศ</a><button type="button" class="button danger" id="delete-post">'+icon("trash")+' ลบประกาศ</button></div>':""}</div>
