...
const postCopy=(post:PostRow)=>{const content=plainPostContent(post.content),limit=220;if(content.length<=limit)return `<div class="post-card-copy"><p>${e(content)}</p></div>`;const preview=content.slice(0,limit).trimEnd();return `<div class="post-card-copy"><p data-post-summary>${e(preview)}…</p><p data-post-full hidden>${e(content)}</p><button class="post-copy-toggle" type="button" data-expand-post aria-expanded="false">ดูเพิ่มเติม</button></div>`;};
...
export function renderPosts(ctx: Context, initialCategory: "official" | "general" | "requests" | "admin" | "approvals") {
  ...
  ctx.root.addEventListener("click",event=>{
    ...
    if(b.hasAttribute("data-expand-post")){const copy=b.closest<HTMLElement>(".post-card-copy"),summary=copy?.querySelector<HTMLElement>("[data-post-summary]"),full=copy?.querySelector<HTMLElement>("[data-post-full]");if(!summary||!full)return;const expanded=b.getAttribute("aria-expanded")==="true";summary.hidden=!expanded;full.hidden=expanded;b.setAttribute("aria-expanded",String(!expanded));b.textContent=expanded?"ดูเพิ่มเติม":"ย่อ";return;}
