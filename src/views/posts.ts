...
import { badge, busy, confirmAction, empty, toast } from "../ui/primitives";
...
const postImage=(post:PostRow,className:string)=>{const url=post.image_url?safeUrl(post.image_url):null;return url?`<img class="${className}" src="${e(url)}" alt="ภาพประกอบ ${e(post.title)}" loading="lazy" decoding="async">`:"";};
export function postCard(post: PostRow) {
  return `<article class="post-card ${post.is_pinned?"pinned":""}"><div class="post-card-head"><div class="post-card-kind">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${post.status!=="published"?badge(postStatusLabel(post),post.status):""}</div><div class="post-card-flags" aria-label="รายวิชา กลุ่มผู้รับ และสถานะ">${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div></div>${postImage(post,"post-card-image")}<h2><a href="${href("postDetail",post.post_id)}">${e(post.title)}</a></h2><p>${e(post.content.replace(/[#*]/g,"").slice(0,150))}${post.content.length>150?"…":""}</p><div class="post-card-footer"><span class="author"><span class="avatar">${e(post.author_name.slice(0,1))}</span><span>${e(post.author_name)}<br><small>${e(formatDate(post.created_at,false))}</small></span></span><a href="${href("postDetail",post.post_id)}" aria-label="อ่าน ${e(post.title)}">${icon("arrow")}</a></div></article>`;
}
...
export function renderPostDetail(ctx:Context){
  ...
  `<div class="detail-grid"><article class="panel"><div class="task-top">${badge(post.category==="official"?"ประกาศทางการ":"ข่าวทั่วไป",post.category)}${badge(postStatusLabel(post),post.status)}${postContext(post)}${post.is_pinned?badge("ปักหมุด","pin"):""}</div><h2 class="detail-title">${e(post.title)}</h2><div class="detail-meta"><span>โดย ${e(post.author_name)}</span><span>${e(formatDate(post.created_at))}</span></div>${postImage(post,"post-detail-image")}<div class="prose">${markdown(post.content)}</div><section class="detail-section"><h3>ลิงก์เอกสารแนบ</h3>${post.attachments.length?post.attachments.map(a=>externalLink(a.url,a.name)).join(""):'<p class="note-hint">ไม่มีเอกสารแนบ</p>'}</section></article>
