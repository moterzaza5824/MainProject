...
export function renderDashboard(ctx:Context,admin=false) {
  ...
  const enrollmentPrompt=!admin&&ctx.enrollments?.length===0?`<div class="onboarding-callout dashboard-onboarding">${icon("book")}<div><h2>ลงทะเบียนรายวิชาก่อนเริ่มใช้งาน</h2><p>เลือกวิชา ปีการศึกษา ภาคเรียน และ Sec เพื่อให้ระบบแสดงเฉพาะงาน ปฏิทิน และประกาศทางการของคุณ</p></div><a class="button primary" href="${href("enrollment")}">ตั้งค่ารายวิชาของฉัน ${icon("arrow")}</a></div>`:"";
  ctx.root.innerHTML=heading(admin?"ภาพรวมผู้ดูแลระบบ":"สวัสดี, "+ctx.user.full_name.split(" ")[0]+" 👋",admin?"จัดการข่าวสารและงานของรุ่นให้อยู่ในที่เดียว":"วันนี้มีอะไรที่ต้องทำบ้าง มาวางแผนการเรียนไปด้วยกัน",admin?"ADMIN WORKSPACE":"YOUR DAILY OVERVIEW",headingActions)+enrollmentPrompt+
  ...
  `<div class="dashboard-grid"><section class="panel"><div class="panel-title"><h2>${admin?"รายการที่รอการตรวจสอบ":"งานที่ต้องจัดการ"}</h2><a class="button small subtle" href="${href(admin?"approvals":"assignments")}">ดูทั้งหมด ${icon("arrow")}</a></div>${admin?
    ...
    ${!admin?'<p class="note-hint" style="margin-top:20px">สรุปตามรายวิชาและ Sec ที่ลงทะเบียนไว้ในหน้ารายวิชาของฉัน</p>':""}</section>
