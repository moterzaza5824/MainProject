...
export function renderCatalog(ctx:Context){
  ...
  `<div class="catalog-layout"><section class="panel catalog-editor"><h2>เพิ่มข้อมูล</h2><p class="note-hint">ข้อมูลต้นแบบจะบันทึกเฉพาะในเบราว์เซอร์นี้จนกว่าจะเชื่อมฐานข้อมูล</p>
    ...
    <form id="subject-catalog-form"><div class="form-error" data-error role="alert" tabindex="-1" hidden></div>
      ...
      <div class="form-row"><label class="field">ปีการศึกษา *<input name="academic_year" type="number" required min="2500" max="2700" value="${academicYear}"></label><label class="field">จำนวน Sec *<input name="section_count" type="number" required min="1" max="20" value="2"><small>ระบุจำนวนกลุ่มเรียนของวิชานี้</small></label></div>
  ...
  const renderLists=()=>{
    ...
    ctx.root.querySelector("#subject-catalog-list")!.innerHTML=catalog.subjects.length?'<div class="catalog-items">'+catalog.subjects.map(row=>`<div class="catalog-item"><div><strong>${e(row.name)}</strong><span>ปีการศึกษา ${row.academicYear}</span></div><b>${row.sectionCount} Sec</b></div>`).join("")+"</div>":'<p class="note-hint">ยังไม่มีรายวิชา กรุณาเพิ่มรายวิชาก่อนสร้างงาน</p>';
  ...
  subjectForm.onsubmit=event=>{
    ...
    try{
      catalog=addSubject(ctx.data.assignments,{name:String(fd.get("name")),academicYear:Number(fd.get("academic_year")),sectionCount:Number(fd.get("section_count"))});
