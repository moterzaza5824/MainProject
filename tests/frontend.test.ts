...
import { matchesSection, taskUrgency, fromThaiInput, thaiInput, formatDate } from "../src/utils/tasks";
...
test("post cards lead with the author, keep context at the top, and place images after the copy", () => {
  ...
  assert.equal(author.querySelector("small")!.textContent,formatDate(post.created_at));
  assert.match(author.querySelector("small")!.textContent!,/11:35/);
